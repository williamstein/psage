...
cpdef eisenstein_series(S,double sigma,double R,double Y,int M,int Q,int gr=0,int use_fak=0,double eps=1e-12,ctx='fp',prec=53):
    r"""
    ...
    RHS=<double complex**>sig_malloc(sizeof(double complex*)*N)
    if RHS==NULL: raise MemoryError
    for n in range(N):
        RHS[n]=NULL
        RHS[n]=<double complex*>sig_malloc(sizeof(double complex)*nc)
    ...
    compute_V_cplx_eis_dp_sym(V,RHS,N,Xm,Xpb,Ypb,Cvec,
                              ...
                              verbose,use_fak,0,mpcontext=mpcontext)   
    sig_off()
    ...
    for i in range(comp_dim):
        C[i]=<double complex*>sig_malloc(sizeof(double complex)*(N))
        if C[i]==NULL:
            raise MemoryError
...
cdef compute_V_cplx_eis_mpc_sym(double complex **V,
                               double complex **RHS,
                           int N,
                           double *Xm,
                           double ***Xpb,
    ...
    mpy=ctx.mpf(Y)
    mpsqY=ctx.sqrt(mpy)
    mpone_minus_s=ctx.mpc(1.0-sigma,-R)
    mp_y_1ms=ctx.power(mpy,mpone_minus_s)
    two=<double>(2)
    Y2pi=Y*pi*two
    twopi=two*pi
    Ml=0; Ql=0
    ...
    kbesvec=<double complex***>sig_malloc(sizeof(double complex**)*nc)
    if kbesvec==NULL:
        raise MemoryError
    for jcusp in range(nc):
        kbesvec[jcusp]=<double complex**>sig_malloc(sizeof(double complex*)*Ml)
        if kbesvec[jcusp]==NULL:
                raise MemoryError
        for l in range(Ml):
            kbesvec[jcusp][l]=<double complex*>sig_malloc(sizeof(double complex)*Ql)
    ...
    s_minus_half_mpc=ctx.mpc(sigma-0.5,R)
                ...
                for l in range(Mv[jcusp][2]):
                    ...
                    if lr<>0.0:
                        if on_half_line:
                            ...
                            try:
                                besarg_mpf = ctx.mpf(besarg)
                                kbes=ctx.besselk(s_minus_half_mpc,besarg_mpf)
                            except:
                                print "s=",s,type(s)
        ...
        for icusp in range(nc):
            for n in range(Mv[icusp][2]):
                ...
                for j in range(Qv[icusp][2]):            
                    ...
                    if ypb == 0:
                        continue
                    term = ctx.mpf(ypb)**s #mpmath.mp.power(mpmath.mp.mpf(ypb),s)
                    if nvec[icusp][n] !=0:
...
cdef compute_V_cplx_eis_dp_sym(double complex **V,
    ...
    prec = 103
    one = float(1)
    Ys = Y**sigma*cos(RlnY)+_I*Y**sigma*sin(RlnY)
    cdef double complex Y_1minus_s
    # Y^(1-s)=Y^(1-sigma)*(cos(Rln(Y))-sin(Rln(Y)))
    Y_1minus_s= Y**(one-sigma)*cos(RlnY)-_I*Y**(one-sigma)*sin(RlnY)
    if verbose>0:
        print "Ys=",Ys
        print "Y**s=",Y**<double complex>(sigma+_I*R)
    if abs(sigma-0.5)<1e-14:
        ...
        for icusp in range(nc):
            for n in range(Mv[icusp][2]):
                ...
                for j in range(Qv[icusp][2]):            
                    ...
                    if ypb == 0:
                        continue
                    term = ypb**s #mpmath.mp.mpf(ypb)**s #mpmath.mp.power(mpmath.mp.mpf(ypb),s)
                    if nvec[icusp][n] !=0:
                        #term = term*CC(0,-nr*Xm[j]).exp()
                        term = term*cexpi(-nr*Xm[j])
                        #term = term*ef2_c[icusp][n][j]
                    summa = summa + term
                    #if jcusp==icusp and nr==0:
                    #    print "term:",jcusp,icusp,nr,j,'=',term
