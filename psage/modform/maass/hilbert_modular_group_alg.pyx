...
cpdef get_closest_cusp_old(Hn z,G,ida=None,int denom_max=3,int verbose=0):
    r""" Locate the closest cusp of G to z.
    if ida<>NOne, it should be an ideal and we want cusps r/s with s in ida
    """
    ...
    cdef list ideal_integral_basis
    if ida<>None:
        if not is_NFIdeal(ida):
            raise ValueError,"Need ideal ida!"
...
cpdef get_closest_cusp_new_with_sl2ao(Hn z,G,ida=None,int denom_max=3,int verbose=0):
    r""" Locate the closest cusp of G to z.
    if ida<>NOne, it should be an ideal and we want cusps r/s with s in ida
    """
    cdef int degree,ns,nr,i,j,nsigmamax
    ...
    cusp_reps = <double***>sage_malloc(nc*sizeof(double**))
    #cusp_reps_coord = <double***>sage_malloc(nc*sizeof(double**))
    cdef double ** denominator_lattice
    denominator_lattice = <double**>sage_malloc(degree*sizeof(double*))
    if ida<>None:
        B0 = G.translation_module(ida)
        BDA = G.translation_module(ida,ret_type='alg')
    else:
        B0 = G.translation_module(0)
        BDA = G.translation_module(0,ret_type='alg')
    for i in range(degree):
        denominator_lattice[i] = <double*>sage_malloc(degree*sizeof(double*))
        for j in range(degree):
            denominator_lattice[i][j]=float(B0[i][j])
    ...
    for i in range(nc):
        ...
        if i==0:
            ...
            if ida<>None:
                if G.cusps()[i].denominator() not in ida:
                    ### Instead of having a true set of representatives we just take a set of cusps which we know are cusps of SL(2,o+a)
                    for j in range(degree):
                        cusp_reps[i][1][j]=denominator_lattice[j][0]
                else:
                    for j in range(degree):
                        cusp_reps[i][1][j]=G.cusps()[i].denominator().complex_embeddings()[j]
            else:
...
#    get_initial_cusp_distance_c(F,xv,yv,ny,degree,rho_v,sigma_v,&d,nc,cusp_reps,cusp_reps_coord,integer_lattice,disc,denom_max,delta0,verbose)
    if ida<>None:
        get_initial_cusp_distance_c_new(xv,yv,ny,degree,rho_v,sigma_v,&d,nc,cusp_reps,integer_lattice,denominator_lattice,disc,denom_max,delta0,verbose)
    else:
        get_initial_cusp_distance_c(xv,yv,ny,degree,rho_v,sigma_v,&d,nc,cusp_reps,integer_lattice,disc,denom_max,delta0,verbose)
    cdef int h = G._K.class_number()
    #cdef NumberFieldElement_quadratic rho_min,sigma_min
    ...
    np = 0
    if ida<>None:
        if sigma_min not in ida:
            raise ArithmeticError("Got cusp with denominator not in the prescribed ideal! Got:{0}".format(sigma_min))
    ...
    if ida<>None:
        ...
        normaid = 1
    nsigmamax = nsigmamax*normaid
    for ns in range(-nsigmamax,nsigmamax):
        if ns==0: continue
        if ns % normaid <>0: continue
        break_sigma=0
        if ida<>None:
            list_of_sigmas =  elements_of_norm_in_ideal(G._K,ida,ns,degree,basis_list)
        else:
            list_of_sigmas =  elements_of_norm(F,ns,degree,basis_embeddings)       
        if verbose>1:
            print "ns=",ns
    ...
    if ida<>None:
        if sigma_min not in ida:
            raise ArithmeticError,"Could not get closest cusp in correct class!"
    if ci<>1:
        g = ci.gens_reduced()[0]
        rho_min = rho_min/g
        sigma_min = sigma_min/g
    ...
    if integer_lattice<>NULL:
        ...
        if integer_lattice[1]<>NULL:
            ...
            sage_free(integer_lattice[1])
        sage_free(integer_lattice)
    if denominator_lattice<>NULL:
        for i in range(degree):
            if denominator_lattice[i]<>NULL:
                sage_free(denominator_lattice[i])
        sage_free(denominator_lattice)
    return c,delta_min
...
cpdef list elements_of_norm_in_ideal(K,ida,int n,int degree,list basis,int check=0):
    global elements_of_F_with_norm_in_ideal
    cdef int i,j
    cdef int numv
    cdef list v,emb
    cdef double x
    #cdef gen a,elts
    if check==1:
        check_cached_field(K)
    if not elements_of_F_with_norm_in_ideal.has_key(ida):
        elements_of_F_with_norm_in_ideal[ida]={}
    if not elements_of_F_with_norm_in_ideal[ida].has_key(n):
        elts = K.elements_of_norm(n) #bnfisintnorm(n)
        elements_of_F_with_norm_in_ideal[ida][n]=[]
        for a in elts:
            if not a in ida:
                continue
            v = a.list()
            ...
            elements_of_F_with_norm_in_ideal[ida][n].append((v,emb))
    return elements_of_F_with_norm_in_ideal[ida][n]
...
cdef get_initial_cusp_distance_c_new(double* x,double *y,double ny,int degree,int *rho_min,int *sigma_min,double* d,int nc,double*** cusp_reps,double*** integer_lattice, double** denominator_lattice,double disc, int denom_max=3,double eps0=1e-12,int verbose=0):
    cdef double *rho=NULL, *sigma=NULL,*xv=NULL,*yv=NULL
    cdef double d0,d1,d2
    cdef int i,j
    ...
    ## First check the cusp representatives
    cdef double dmin = 0.0
    # We don't yet have a set of reps for cusps of SL2(a+O) 
    # So we simply go through the ones for SL(2,O) and make
    # a test in the calling routine to make sure that we only look at  cusps of SL(2,a+Z)
    cdef gen idp
    for j in range(nc):
        for i in range(degree):
            rho[i]=cusp_reps[j][0][i]
    ...
    for q in range(1,denom_max):
        ...
        for v in pcoeffs:
            for i in range(degree):
                ...
                for j in range(degree):
                    rho[i]+=v[j]*integer_lattice[0][i][j]
                sigma[i]=<double>q*denominator_lattice[i][0] 
            #for i in range(degree):
            #    vr[i]=v[i]; vs[i]=sigma[i]
            ###  
            norm = 1.0 #get_ideal_norm(F,vr,vs)
            ...
            if d2<dmin-eps0:
                ...
                for i in range(degree):
                    ...
                    if i>0:
                        ...
                        # The first column should be integral
                        sigma_min[0]=q*int(denominator_lattice[i][0])  #sigma[i]
