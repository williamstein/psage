...
cdef Gauss_elim_filter(mpc_t** A,mpc_t** B,int N,int num_rhs,int num_set,mpc_t** C, int* filter,int verbose=0):
    ...
    for j in range(N):
        ...
        if do_cont==1:
            ...
            used[j]=0
        #print "used[{0}]={1}".format(j,used[j])
    for m in range(N):
        if used[m]==-1:
            continue
        mpfr_set_ui(rtemp,0,rnd_re)
        ...
        for j in range(N):
            ...
            for k in range(m+1,N):
                ...
                mpc_sub(A[j][k],A[j][k],ctemp,rnd)
            for k in range(num_rhs):
                mpc_mul(ctemp,B[maxi][k],A[j][m],rnd)
                mpc_sub(B[j][k],B[j][k],ctemp,rnd)                
    ...
    for i in range(num_rhs):
        ...
        for m in range(N): #DO M=1,N
            if used[m]==-1:
                ...
                mpc_set(C[i][m],B[piv[m-roffs]][i],rnd)
            if verbose>1:
                mpc_set(tempc.value,C[i][m],rnd)
                print "Set C[{0}] from system to {1}".format(m,tempc)
...
cpdef test_lin_solve_filter(Matrix_complex_dense A,Matrix_complex_dense RHS,dict setC,list skipping,int ncusps,int Ms,int Mf,int verbose=0):
    r"""
    ...
    Gauss_elim_filter(A._matrix,RHS._matrix,nrow,nrhs,index_to_skip,C,setc_ix,verbose)
    if verbose>0:
        print "after!"
    ...
    for j in range(nrhs):        
        ...
        for r in range(ncusps):
            ...
            for n in range(Ml):
                ...
                tempc2._parent = CF
                if verbose>0:
                    print "C[{0}][{1}]={2}".format(r,n,tempc)
                Cret[j][r][n]=tempc2
