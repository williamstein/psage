...
_jdb = None
def jCurveEulerTables(read_only=True):
    r"""
...
_ldb = None
def LocalEulerTables(read_only=True):
    r"""
    ...
    def _save_euler_table(self, n, verbose=False):
        ...
        # work with user's repository of euler tables
            ...
            if os.path.exists(fname):
                dictionary = load(fname)
            else:
                dictionary = {}
            entry = {n:self.__euler_table(n)}
            if dictionary.has_key(ainvs_pairs):
                if dictionary[ainvs_pairs].has_key(q):
                    dictionary[ainvs_pairs][q].update(entry)
                else:
                    entry = {q:entry}
                    dictionary[ainvs_pairs].update(entry)
            else:
                entry = {q:entry}
                dictionary.update({ainvs_pairs:entry})
            save(dictionary, fname)
            """

    def _load_euler_table(self, n, verbose=False):
        r"""
        ...
        if ainvs == [0,0,0,-27*t*(t-1728)**3,54*t*(t-1728)**5]:
            ...
            if not os.path.exists(SAGE_ROOT + '/data/jcurve_euler_tables/jcurve_euler_tables'):
                ...
                self._set_euler_table(n, euler_db[q][n])
        ...
        # work with user's repository of euler tables
            ...
            if not os.path.exists(SAGE_ROOT + '/data/local_euler_tables/local_euler_tables'):
                ...
                # check that keys exist?
                self._set_euler_table(n, local_euler_db[ainvs_pairs][q][n])
